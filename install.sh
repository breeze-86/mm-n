...
echo "\e[1m[1/7]Installing cpulimit...\e[0m"
apt-get update -y
apt-get install -y cpulimit
echo ""

# Step 2: Configure Linux Network Device Settings
echo "\e[1m[2/7]Configuring network device settings...\e[0m"
...
echo "\e[1m[3/7]Downloading the source code...\e[0m"
mkdir -p /opt && cd /opt
...
echo "\e[1m[4/7]Setting up logrotate...\e[0m"
...
echo "\e[1m[5/7]Starting the node...\e[0m"
cd ${ROOT_DIR}/node
...
echo "\e[1m[6/7]Please modify the configuration file at ${ROOT_DIR}/node/.config/config.yml with the following settings\e[0m"
echo "To enable gRPC:"
